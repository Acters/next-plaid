...
/// Optional model metadata; its absence does not make a snapshot unusable.
const OPTIONAL_FILES: &[&str] = &["config.json"];
...
pub fn ensure_model(model_id: Option<&str>, _quiet: bool) -> Result<PathBuf> {
    ...
    // config.json is optional on some repositories.
    for file in OPTIONAL_FILES {
        let _ = repo.get(file);
    }
...
mod tests {
    ...
    fn make_complete_cache(temp: &TempDir, model_id: &str) -> Cache {
        let files: Vec<&str> = REQUIRED_FILES
            .iter()
            .chain(WEIGHT_FILES.iter())
            .copied()
            .collect();
        make_cache(temp, model_id, &files)
    }
    ...
    fn ensure_model_returns_exact_current_canonical_snapshot() {
        ...
        let cache = make_complete_cache(&temp, model_id);
    ...
    #[test]
    fn cache_accepts_a_single_weight_variant_and_rejects_none() {
        for weight in WEIGHT_FILES {
            let temp = tempfile::tempdir().unwrap();
            let model_id = "owner/one-weight";
            let files: Vec<&str> = REQUIRED_FILES
                .iter()
                .copied()
                .chain(std::iter::once(*weight))
                .collect();
            let cache = make_cache(&temp, model_id, &files);
            assert!(cached_snapshot(model_id, &cache).is_some());
        }

        let temp = tempfile::tempdir().unwrap();
        let model_id = "owner/no-weights";
    ...
    fn cache_accepts_realistic_hf_blob_symlinks() {
        ...
        for file in REQUIRED_FILES.iter().chain(WEIGHT_FILES.iter()) {
