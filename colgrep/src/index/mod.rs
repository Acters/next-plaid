...
impl Searcher {
    ...
    pub fn filter_by_path_prefix(&self, prefix: &Path) -> Result<Vec<i64>> {
        ...
        if prefix_str.is_empty() || prefix_str.ends_with('/') || prefix_str.ends_with('\\') {
            anyhow::bail!("subdirectory filter must name a normalized relative directory");
        }

        // Match the exact directory or one of its descendants on a whole path
        // component: a bare string prefix would also pull in sibling directories
        // sharing the string prefix (`corpus` ⊃ `corpus-extra/`). Stored paths
        // use native separators (serde serializes the unit's PathBuf), so the
...
mod tests {
    ...
    fn test_scan_project_files_merges_force_included_dir() {
        ...
        let (without, _) = scan_project_files(&root, None, &[], &[], &[], false).unwrap();
        assert_eq!(without, vec![PathBuf::from("tracked.md")]);

        let (mut with, _) =
            scan_project_files(&root, None, &[], &[], &[PathBuf::from("corpus")], false).unwrap();
        with.sort();
        assert_eq!(
            with,
            vec![
