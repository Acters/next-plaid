...
/// Validate and map an existing merged inverse-norm sidecar without modifying
/// the index directory.
///
/// This mirrors the fast-path checks in `merge_inv_norm_chunks` (manifest
/// freshness against `metadata.json`, chunk count, padding, and row totals)
/// but never merges, rewrites, or prewarms anything. The sidecar is optional:
/// any absence, staleness, or corruption yields `None` so asymmetric search
/// falls back to per-document norm computation, which is correct but slower.
/// `expected_rows` is the already-validated merged codes row count
/// (`total_doc_tokens + padding_needed`).
fn load_merged_inv_norms_read_only(
    index_dir: &Path,
    num_chunks: usize,
    padding_rows: usize,
    expected_rows: usize,
) -> Option<crate::mmap::MmapNpyArray1F32> {
    let merged_path = index_dir.join("merged_inv_norms.npy");
    let manifest_path = index_dir.join("merged_inv_norms.manifest.json");
    let manifest: crate::mmap::MergeManifest =
        serde_json::from_reader(BufReader::new(File::open(&manifest_path).ok()?)).ok()?;

    let metadata_mtime = fs::metadata(index_dir.join("metadata.json"))
    ...
    if manifest.num_chunks != num_chunks
        ...
        || (manifest.metadata_mtime - metadata_mtime).abs() >= 0.001
        || !merged_path.exists()
    {
        return None;
    }
...
impl MmapIndex {
    ...
    fn load_with_mode(index_path: &str, read_only: bool) -> Result<Self> {
        ...
        let (mmap_codes, mmap_residuals, mmap_inv_norms) = if read_only {
            ...
            // Optionally map an existing, valid inverse-norm sidecar. A
            // read-only load never merges, prewarms, or writes sidecar files;
            // when the sidecar is absent or stale the index keeps `None` and
            // search falls back to per-document norm computation.
            let mmap_inv_norms = if metadata.binary {
                None
            } else {
                load_merged_inv_norms_read_only(
                    index_dir,
                    metadata.num_chunks,
                    padding_needed,
                    mmap_codes.len(),
            ...
            (mmap_codes, mmap_residuals, mmap_inv_norms)
        } else {
        ...
        //
        // A read-only load must never create, merge, or prewarm sidecar
        // files, so it skips this upgrade entirely and keeps the fallback.
        if !read_only {
